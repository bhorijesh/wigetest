...
class AccessPro {
    constructor({
        ...
        this.widgetSize = localStorage.getItem("acp-widget-size") || settings.widget_size || 'medium';
    ...
    renderWidget() {
      ...
      <button class="acp-compliance" data-ap-translate="OUR MISSION">Our Mission</button>
    ` : '';

        const resetButton = reset ? `
      <button class="acp-reset" data-ap-translate="RESET TITLE">Reset</button>
    ` : '';

        const brandingLink = removeBranding ? '' : `
      <a href="https://apps.shopify.com/access-pro" target="_blank" class="acp-branding-link">
        <span class="acp-branding">
          <span class="acp-branding-icon">
            <svg xmlns="http://www.w3.org/2000/svg"  style="height: 20px; width: 20px; display: flex" viewBox="0 0 512 512"><path d="M256,112a56,56,0,1,1,56-56A56.06,56.06,0,0,1,256,112Z" /><path d="M432,112.8l-.45.12h0l-.42.13c-1,.28-2,.58-3,.89-18.61,5.46-108.93,30.92-172.56,30.92-59.13,0-141.28-22-167.56-29.47a73.79,73.79,0,0,0-8-2.58c-19-5-32,14.3-32,31.94,0,17.47,15.7,25.79,31.55,31.76v.28l95.22,29.74c9.73,3.73,12.33,7.54,13.6,10.84,4.13,10.59.83,31.56-.34,38.88l-5.8,45L150.05,477.44q-.15.72-.27,1.47l-.23,1.27h0c-2.32,16.15,9.54,31.82,32,31.82,19.6,0,28.25-13.53,32-31.94h0s28-157.57,42-157.57,42.84,157.57,42.84,157.57h0c3.75,18.41,12.4,31.94,32,31.94,22.52,0,34.38-15.74,32-31.94-.21-1.38-.46-2.74-.76-4.06L329,301.27l-5.79-45c-4.19-26.21-.82-34.87.32-36.9a1.09,1.09,0,0,0,.08-.15c1.08-2,6-6.48,17.48-10.79l89.28-31.21a16.9,16.9,0,0,0,1.62-.52c16-6,32-14.3,32-31.93S451,107.81,432,112.8Z" /></svg>
          </span>
          <span class="acp-branding-text">
           
            <span class="acp-branding-name">AccessPro</span>
          </span>
        </span>
      </a>
    `;
      ...
      <div id="languageWrapper" class="acp-language"></div>
    ` : '';
    ...
    `;
      ...
      <div class="accesspro asw-menu"> 
        ...
        <div class="acp-body">
          ...
          <div class="acp-widget-size">
              <span data-ap-translate="WIDGET SIZE">Widget Size</span>
              <select id="acp-widget-size" class="acp-widget-size-select">
                <option value="small" ${this.widgetSize === 'small' ? 'selected' : ''} data-ap-translate="SMALL">Small</option>
                <option value="large" ${this.widgetSize === 'large' ? 'selected' : ''} data-ap-translate="MEDIUM">Medium</option>
                <option value="larger" ${this.widgetSize === 'larger' ? 'selected' : ''} data-ap-translate="LARGE">Large</option>
              </select>
          </div>
          ` : ''}
          ${this.enableMenuSettings?.keyboardshortcuts ? `
          <div class="acp-shortcuts">
            <button class="acp-shortcut-button" id="acp-shortcut-button">
      ...
      <div id="popoverContainer" class="acp-popover acp-hidden">
        <div class="acp-popover-header">
          ...
          <button class="acp-popover-close">
    ...
    addHideWidgetButton() {
        ...
        const hideWidgetButton = document.createElement('button');
    ...
    `;
        ...
        hideWidgetButton.addEventListener('click', () => {
          ...
          <div class="acp-hide-widget-options">
            <span data-ap-translate="HIDE WIDGET DESC">How long would you like to hide the widget?</span>
            <select id="hide-duration-select" class="acp-hide-duration-select">
              <option value="session" data-ap-translate="FOR CURRENT SESSION">For current session</option>
              <option value="day" data-ap-translate="FOR 24 HOURS">For 24 hours</option>
              <option value="week" data-ap-translate="FOR ONE WEEK">For one week</option>
              <option value="forever" data-ap-translate="FOREVER">Forever (until local storage is cleared)</option>
            </select>
            <div style="text-align: right; margin-top: 20px;">
              <button id="confirm-hide-widget" class="acp-hide-option" data-ap-translate="HIDE WIDGET">Hide Widget</button>
            </div>
          </div>
        `
            });
    ...
    attachEventListeners() {
        ...
        addEventListener('.acp-compliance', 'click', () => this.renderAccessibilityMission());
        addEventListener('.acp-reset', 'click', () => this.resetWidget());
        addEventListener('#acp-shortcut-button', 'click', () => this.toggleShortcuts());
        addEventListener('#acp-widget-size', 'change', (e) => this.handleWidgetSizeChange(e));
    }
    ...
    addLanguage(language) {
        if (!language || !this.enableMenuSettings?.languages) {
            return;
        }
    ...
    showPopover(title, content, isHTMLElement = false) {
    ...
    handleWidgetSizeChange(e) {
        ...
        if (widgetContainer) {
            ...
            widgetContainer.classList.add(`acp-widget-${selectedSize}`);
            localStorage.setItem('acp-widget-size', selectedSize);
        }
    }
    ...
    renderAccessibilityMission() {
        const title = this.complianceData?.compliance_title || 'Our Mission';
...
• Contact Us: Reach out via the website's contact form for queries or concerns.`
    ...
    resetWidget() {
        ...
        if (widgetContainer) {
            ...
            // if there is widget size select, reset it to default
            const widgetSizeSelect = document.querySelector('#acp-widget-size');
            if (widgetSizeSelect) {
                widgetSizeSelect.value = widgetSize;
            }
...
class AccessProPageStructure extends AccessProFeature {
    ...
    activate() {
        ...
        this.widget.showPopover(popoverTitle, tabsWrapper, true)
    }
    ...
    populateHeadings(container) {
        ...
        headings.forEach(heading => {
            let textContent = heading?.textContent.trim() || "(No Text)";
            const headingItem = this.createPageStructureItem(
                `<span class="asw-heading-tag">${heading.tagName.toLowerCase()}</span> ${textContent}`
            );
    ...
    populateLinks(container) {
        ...
        links.forEach(link => {
            ...
            let linkText = link.textContent.trim() || "(No Text)";
...
class VoiceNavigationFeature extends AccessProFeature {
    ...
    <div class="voice-bar-right">
      <button id="acp-voice-start-button" class="acp-voice-start-button" aria-label="Start voice control" style="display: none;">Start</button>
      <button id="voice-help" class="voice-bar-help-button" aria-label="Show help">
        Need help?
      </button>
      <button id="voice-close" class="asw-text-bar-close" aria-label="Close voice navigation">
    ...
    return new Promise((resolve, reject) => {
        try {
            accessProIconScript.onload = async () => {
                ...
                const contentSettingsContainer = document.getElementById('contentSettingsContainer');

                const browsingSettingsContainer = document.getElementById('browsingSettingsContainer');
                ...
                if (enableMenuSettings?.fontscale) {
                    ...
                    if (settings?.template === "default") {
                        ...
                        widget.addFeature(fontScaleFeature, contentSettingsContainer);
                        ...
                        widget.addFeature(accessProFontScale, contentSettingsContainer);
                        accessProVisuallyImpairedProfileFeatures.push({
                ...
                if (enableMenuSettings.dyslexic) {
                    ...
                    widget.addFeature(accessProDyslexiaFont, contentSettingsContainer);
                }

                if (enableMenuSettings.headings) {
                    const accessProLineSpacing = new AccessProLineSpacing();
                    widget.addFeature(accessProLineSpacing, contentSettingsContainer);
                    accessProVisuallyImpairedProfileFeatures.push({
                ...
                if (enableMenuSettings.letterspacing) {
                    ...
                    widget.addFeature(accessProLetterSpacing, contentSettingsContainer);
                }

                if (enableMenuSettings.fontweight) {
                    const accessProFontWeight = new AccessProFontWeight();
                    widget.addFeature(accessProFontWeight, contentSettingsContainer);
                }

                if (enableMenuSettings.alignment) {
                    const accessProTextAlignment = new AccessProTextAlignment();
                    widget.addFeature(accessProTextAlignment, contentSettingsContainer);
                }

                if (enableMenuSettings.contrast) {
                    const accessProContrastMode = new AccessProContrastMode();
                    widget.addFeature(accessProContrastMode, browsingSettingsContainer);
                    accessProElderlyProfileFeatures.push({
                ...
                if (enableMenuSettings.saturation) {
                    ...
                    widget.addFeature(accessProColorSaturation, browsingSettingsContainer);
                ...
                widget.addFeature(accessProColorFilters, browsingSettingsContainer);
                accessProColorBlindProfileFeatures.push({
                ...
                if (enableMenuSettings.invertcolors) {
                    ...
                    widget.addFeature(accessProInvertColors, browsingSettingsContainer);
                }

                if (enableMenuSettings.cursor) {
                    const accessProBigCursor = new AccessProBigCursor();
                    widget.addFeature(accessProBigCursor, browsingSettingsContainer);
                    accessProMotorImpairedProfileFeatures.push({
                ...
                if (enableMenuSettings.magnifier) {
                    ...
                    widget.addFeature(accessProTextMagnifier, browsingSettingsContainer);
                    accessProMotorImpairedProfileFeatures.push({
                ...
                if (enableMenuSettings.readingguide) {
                    ...
                    widget.addFeature(accessProReadingGuide, browsingSettingsContainer);
                }

                if (enableMenuSettings.readingmask) {
                    const accessProReadingMask = new AccessProReadingMask();
                    widget.addFeature(accessProReadingMask, browsingSettingsContainer);
                    accessProADHDProfileFeatures.push({
                ...
                if (enableMenuSettings.links) {
                    ...
                    widget.addFeature(accessProHighlightLinks, browsingSettingsContainer);
                ...
                if (enableMenuSettings.headings) {
                    ...
                    widget.addFeature(accessProHighlightHeadings, browsingSettingsContainer);
                    accessProCognitiveProfileFeatures.push({
                ...
                if (enableMenuSettings.sounds) {
                    ...
                    widget.addFeature(accessProMuteSound, browsingSettingsContainer);
                }

                if (enableMenuSettings.animation) {
                    const accessProStopAnimation = new AccessProStopAnimation();
                    widget.addFeature(accessProStopAnimation, browsingSettingsContainer);
                ...
                if (enableMenuSettings.images) {
                    ...
                    widget.addFeature(accessProHideImages, browsingSettingsContainer);
                    accessProReaderProfileFeatures.push({
                ...
                if (enableMenuSettings.videos) {
                    ...
                    widget.addFeature(accessProHideVideo, browsingSettingsContainer);
                    accessProReaderProfileFeatures.push({
                ...
                if (accessProPlanName !== "free") {
                    if (!enableMenuSettings.translate) {
                        ...
                        widget.addFeature(accessProTranslatePage, browsingSettingsContainer);
                    }

                    if (enableMenuSettings.describeimages) {
                        const accessProDescribeImage = new AccessProDescribeImage();
                        widget.addFeature(accessProDescribeImage, browsingSettingsContainer);
                    }


                    if (enableMenuSettings.text2speech) {
                        accessProTextToSpeech = new AccessProTextToSpeech();
                        widget.addFeature(accessProTextToSpeech, browsingSettingsContainer);
                ...
                if (enableMenuSettings.customcolors) {
                    ...
                    widget.addFeature(colorsFeature, contentSettingsContainer);
                }
                ...
                if (enableMenuSettings.linknavigation) {
                    ...
                    widget.addFeature(accessProLinkNavigatorFeature, contentSettingsContainer);
                }


                if (enableMenuSettings.voicenavigation) {
                    const accessProVoiceNavigationFeature = new VoiceNavigationFeature();
                    widget.addFeature(accessProVoiceNavigationFeature, contentSettingsContainer);
                }


                if (enableMenuSettings.talkandtype) {
                    const accessProTalkAndTypeFeature = new AccessProTalkAndType();
                    widget.addFeature(accessProTalkAndTypeFeature, contentSettingsContainer);
                }
...
class AccessProTalkAndType extends AccessProFeature {
    ...
    renderTalkAndTypeBar() {
      ...
      <div class="talk-type-bar-right">
        <button id="acp-talk-type-start-button" class="acp-voice-start-button" aria-label="Start talk and type" style="display: none;">Start</button>
        <button id="talk-type-help" class="talk-type-bar-help-button" aria-label="Show help">Help</button>
        <button id="talk-type-close" class="asw-text-bar-close" aria-label="Close talk and type">${ACCESSPRO_ICONS.xMark}</button>
      </div>
    `;
        document.body.appendChild(bar);
    ...
    _init() {
        ...
        <div class="acp-help-modal" role="dialog" aria-labelledby="${this.id}-title" aria-modal="true">
          <div class="acp-help-modal-header">
            ...
            <button class="acp-help-modal-close" aria-label="Close help dialog">
              ${ACCESSPRO_ICONS.xMark}
